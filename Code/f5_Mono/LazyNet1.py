...
dat1 = pd.read_csv('trainset140_bigc_quad_7_053024.csv', header=None)
dat1 = dat1.to_numpy()
...
class LazyNet1(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            ...
            exp_layer = torch.exp(dense_exp_output.clamp(max=20))
...
optimizer = optim.Adam(model.parameters(), lr=LRN)
loss_values = []
average_loss_values = []
...
for epoch in range(enumber):
    ...
    for inputs in train_loader:
        ...
        num_batches += 1
    average_epoch_loss = running_loss / num_batches
...
modelname = str(runcount)+'_LazyNet1_bigquad_'+str(width)+'X'+str(depth)+'Xe'+str(enumber)+'bsize'+str(bsize)+'@'+str(LRN)+'dk'+str(dcay)+'_nature_cuda'+str(gnum)+".pth"
